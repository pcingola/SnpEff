...
public class SnpEffCmdPdb extends SnpEff {
	 ...
	 */
	List<IdMapperEntry> checkSequencePdbGenome(Structure pdbStruct, Chain chain, String trId,
			List<IdMapperEntry> idmapsOri) {
	 ...
	 */
	List<DistanceResult> findInteractingCompound(Structure pdbStruct, Chain chain1, Chain chain2, String trId1,
			String trId2) {
				...
				for (IdMapperEntry im1 : idMapChain1) {
					for (IdMapperEntry im2 : idMapChain2) {
						...
						if (!im1.trId.equals(im2.trId)) {
							List<DistanceResult> dres = findInteractingCompound(pdbStruct, chain1, chain2, im1.trId,
									im2.trId);
							save(dres);
						}
					}
	...
	public void usage(String message) {
		...
		System.err.println("\nOptions:");
		System.err.println(
				"\t-aaSep <number>                 : Minimum number of AA of separation within the sequence. Default: "
						+ aaMinSeparation);
		System.err.println(
				"\t-idMap <file>                   : ID map file (i.e. file containing mapping from PDB ID to transcript ID). Default: "
						+ idMapFile);
		System.err.println(
				"\t-interactList <file>            : A file containing protein-protein interations (from PDB co-srystalzed structures). Default: "
						+ interactListFile);
		System.err.println(
				"\t-maxDist <number>               : Maximum distance in Angtrom for any atom in a pair of amino acids to be considered 'in contact'. Default: "
						+ distanceThreshold);
		System.err.println(
				"\t-maxErr <number>                : Maximum amino acid sequence differece between PDB file and genome. Default: "
						+ maxMismatchRate);
		System.err.println("\t-org <name>                     : Organism 'common name'. Default: " + pdbOrganismCommon);
		System.err.println(
				"\t-orgScientific <name>           : Organism 'scientific name'. Default: " + pdbOrganismScientific);
		System.err
				.println("\t-pdbDir <path>                  : Path to PDB files (files in all sub-dirs are scanned).");
		System.err
				.println("\t-res <number>                   : Maximum PDB file resolution. Default: " + pdbResolution);

...
public class SnpEffCmdPdb extends SnpEff {
	...
	Map<String, String> chainUniprotIds(Structure pdbStruct) {
		...
		for (DBRef dbref : pdbStruct.getDBRefs()) {
			if (debug) Log.debug("PDB_DBREF\tchain:" + dbref.getChainName() + "\tdb: " + dbref.getDatabase() + "\tID: " + dbref.getDbAccession());
	...
	boolean filterPdbChain(Chain chain) {
		...
		for (EntityInfo entityInfo : chain.getStructure().getEntityInfos()) {
			if (contains(entityInfo.getOrganismCommon(), pdbOrganismCommon) || contains(entityInfo.getOrganismScientific(), pdbOrganismScientific)) { return true; }
		}
		return false;
	}
			...
			for (AminoAcid aa2 : aas2) {
				...
				if (select(dmin)) {
					...
					if (dres.hasValidCoords()) {
						results.add(dres);
						countMapOk++;
						if (debug) Log.debug(((dmin <= distanceThreshold) ? "AA_IN_CONTACT\t" : "AA_NOT_IN_CONTACT\t") + dres);
					} else {
						countMapError++;
					}
				}
			...
			for (int j = minj; j < aas.size(); j++) {
				...
				if (select(d)) {
					...
					if (dres.hasValidCoords()) {
						results.add(dres);
						countMapOk++;
						if (debug) Log.debug(((d <= distanceThreshold) ? "AA_IN_CONTACT\t" : "AA_NOT_IN_CONTACT\t") + dres);
					} else {
						countMapError++;
					}
				}
	...
	public void initialize() {
		...
		for (Gene g : config.getSnpEffectPredictor().getGenome().getGenes())
			for (Transcript tr : g) {
				String id = tr.getId();
				if (id.indexOf('.') > 0) id = id.substring(0, id.indexOf('.')); // When using RefSeq transcripts, we don't store sub-version
																				// number

...
$SCRIPTS_BUILD_DIR/hg38_RefSeq_NCBI_fix_GFF.pl genes.ORI.gff $CHR_IDS_2_NAME > $DB_DIR/genes.gff 
gzip $DB_DIR/genes.gff

echo "Processing reference FASTA files"
gunzip -c hs_ref*.fa.gz | $SCRIPTS_BUILD_DIR/hg38_RefSeq_NCBI_fix_FASTA.pl $CHR_IDS_2_NAME | $SCRIPTS_DIR/fastaSplit.pl 
...
cat protein.ORI.fa | $SCRIPTS_BUILD_DIR/hg38_RefSeq_NCBI_fix_FASTA_ProCds.pl protein_id.map.txt > $DB_DIR/protein.fa
gzip $DB_DIR/protein.fa

echo "Processing RNA FASTA files"
gunzip -c human.?.rna.fna.gz | sed "s/^>gi|[0-9]*|ref|\(.*\)|.*/>\1/" > cds.ORI.fa 
cat cds.ORI.fa | $SCRIPTS_BUILD_DIR/hg38_RefSeq_NCBI_fix_FASTA_ProCds.pl ids.map.txt > $DB_DIR/cds.fa
gzip $DB_DIR/cds.fa
